      ...
      <main className="flex-1 py-4 md:py-8">
        <div className="container mx-auto px-3 md:px-4">
          ...
          <div className="text-center mb-8 md:mb-12">
            <h1 className="text-xl md:text-4xl font-bold mb-3 md:mb-4">Referências Científicas</h1>
          ...
          <div className="mb-12 md:mb-16">
            <h2 className="text-lg md:text-2xl font-bold text-center mb-6 md:mb-8">Fontes de Referência</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 max-w-6xl mx-auto">
              {referenceCategories.map((category) => (
                <Card key={category.title} className="hover:shadow-lg transition-shadow">
                  <CardHeader className="pb-3 md:pb-6">
                    <div className="flex items-center gap-2 md:gap-3">
                      <div className={`p-1.5 md:p-2 rounded-md md:rounded-lg ${category.color}`}>
                        <category.icon className="h-4 w-4 md:h-5 md:w-5" />
                      </div>
                      <div>
                        <CardTitle className="text-sm md:text-lg">{category.title}</CardTitle>
                        <CardDescription className="text-xs md:text-base">{category.description}</CardDescription>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent className="pt-0">
                    <div className="space-y-2 md:space-y-3">
                      {category.references.map((ref, index) => (
                        <div key={index} className="border-l-2 border-primary/20 pl-3 md:pl-4">
                          <div className="flex items-start justify-between gap-2">
                            <div className="flex-1">
                              <h4 className="font-medium text-xs md:text-sm">{ref.title}</h4>
          ...
          <div className="mb-12 md:mb-16">
            <h2 className="text-lg md:text-2xl font-bold text-center mb-6 md:mb-8">
              Referências Específicas por Categoria
            </h2>
                ...
                <Card key={section.category}>
                  <CardHeader className="pb-3 md:pb-6">
                    <div className="flex items-center gap-2">
                      ...
                      <CardTitle className="text-base md:text-xl">{section.category}</CardTitle>
                    </div>
                  </CardHeader>
                  <CardContent className="pt-0">
                    <div className="space-y-2 md:space-y-3">
                      {section.references.map((reference, index) => (
                        <div key={index} className="flex items-start gap-2 md:gap-3">
                          ...
                          <p className="text-xs md:text-sm text-muted-foreground leading-relaxed">{reference}</p>
                        </div>
                      ))}
                    </div>
                  </CardContent>
          ...
          <div className="mb-12 md:mb-16">
            <Card className="max-w-4xl mx-auto bg-gradient-to-br from-primary/5 to-accent/5">
              <CardHeader className="text-center pb-3 md:pb-6">
                <CardTitle className="text-lg md:text-2xl">Metodologia de Seleção</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-3 md:space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                    <div className="space-y-2">
                      <h4 className="font-semibold text-primary text-sm md:text-base">Critérios de Inclusão:</h4>
                    ...
                    <div className="space-y-2">
                      <h4 className="font-semibold text-primary text-sm md:text-base">Fontes Priorizadas:</h4>
                      <ul className="text-xs md:text-sm text-muted-foreground space-y-1">
          ...
          <div className="text-center">
            <Card className="max-w-3xl mx-auto bg-muted/30">
              <CardContent className="pt-4 md:pt-6">
                <h3 className="text-base md:text-lg font-semibold mb-3 md:mb-4">Importante</h3>
